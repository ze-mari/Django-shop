...
class CategoryManager(models.Manager):
    ...
    def get_categories_for_left_sidebar(self):
        category_models = get_models_for_count('notebook', 'smartphone')
        qs = list(self.get_queryset().annotate(*category_models))
        data = [
...
class Product(models.Model):
    ...
    #     #     raise MinResolutionException('Разрешение изоброжения меньше минимального!')
    #     # if img.height > max_height or img.width > max_width:
    #     #     raise MaxResolutionException('Разрешение изоброжения больше максимального!')
    #     img = Image.open(self.image)
    #     new_img = img.convert('RGB')
    #     resized_img = new_img.resize((400, 400), Image.ANTIALIAS)
    #     filestream = BytesIO()
...
class Notebook(Product):
    ...
    display_type = models.CharField(max_length=255, verbose_name="Тип дисплейа")
    processor_freq = models.CharField(max_length=255, verbose_name="Частота процессора")
    ram = models.CharField(max_length=255, verbose_name="Оперативная память")
    video = models.CharField(max_length=255, verbose_name="Видекарта")
...
class Smartphone(Product):
    ...
    sd_volume = models.CharField(max_length=255, null=True, blank=True, verbose_name="Объем сд карты")
